...
const radarOptionsEl=$("#radar-options");
const radarTimezoneEl=$("#radar-timezone");
const radarDateEl=$("#radar-date");
const radarTimeEl=$("#radar-time");
const radarRegionEl=$("#radar-region");

function todayJST(){ return new Date(Date.now()+9*3600*1000).toISOString().slice(0,10); }
dateEl.value = todayJST();
if(radarDateEl) radarDateEl.value = dateEl.value;
if(radarTimeEl) radarTimeEl.value = `${hourEl.value}:00`;
...
function radarSelected(){
  return !!document.querySelector('[data-chart="RADAR"]:checked');
}

function updateRadarOptions(){
  if(radarOptionsEl) radarOptionsEl.hidden=!radarSelected();
}

function radarDateObject(){
  const date=radarDateEl?.value || dateEl.value;
  const time=radarTimeEl?.value || `${hourEl.value}:00`;
  return radarTimezoneEl?.value==="JST"
    ? new Date(`${date}T${time}:00+09:00`)
    : new Date(`${date}T${time}:00Z`);
}

function radarTimeAsJSTStamp(){
  const d=radarDateObject();
  const j=new Date(d.getTime()+9*3600*1000);
  return `${j.getUTCFullYear()}${pad(j.getUTCMonth()+1)}${pad(j.getUTCDate())}${pad(j.getUTCHours())}${pad(j.getUTCMinutes())}`;
}

function radarDisplayTimes(){
  const d=radarDateObject();
  return {utc:fmt(d),jst:fmtJST(d)};
}
...
function renderCharts(){
  ...
  const selectedHour = Number(hourEl.value);
  listEl.innerHTML = "";
  ...
  for(const [g,arr] of Object.entries(groups)){
    ...
    for(const c of arr){
      const available = c.validHours.includes(selectedHour);

      const ft=c.containedForecastHours?.length
        ? `収録予報時間: ${c.containedForecastHours.map(x=>"+"+x+"h").join(" / ")}`
        : `対応時刻: ${c.validHours.map(x=>pad(x)+" UTC").join(", ")}`;

      const lab=document.createElement("label");
      lab.className=`item${available ? "" : " unavailable"}`;
      ...
      lab.innerHTML=`
        ...
        <span>
          <b>${c.id}</b> ${c.name}
          <small>${ft}${available ? "" : "　※この時刻では選択できません"}</small>
        </span>`;
...
function updateHourEmphasis(){
  const isMajor = hourEl.value==="00" || hourEl.value==="12";
  hourEl.classList.toggle("major-selected", isMajor);
}
...
function makePlan(c){
  ...
  if(c.source==="radar"){
    const rt=radarDisplayTimes();
    note=`レーダー: ${rt.utc} / ${rt.jst}、地域: ${radarRegionEl?.value || "全国"}`;
  }
...
async function fetchPlan(p){
  ...
  if(c.source==="radar"){
    return getBytes(proxy("/radar",{
      datetime:radarTimeAsJSTStamp(),
      region:radarRegionEl?.value || "全国"
    }));
  }
  if(c.source==="sunny"){
...
radarTimezoneEl?.addEventListener("change",renderPlan);
radarDateEl?.addEventListener("change",renderPlan);
radarTimeEl?.addEventListener("change",renderPlan);
radarRegionEl?.addEventListener("change",renderPlan); dateEl.addEventListener("change",renderPlan); hourEl.addEventListener("change",renderCharts); modeEl.addEventListener("change",renderCharts); $("#build").addEventListener("click",build); renderCharts();
