...
radarDateEl.value = dateEl.value;
radarTimeEl.value = `${hourEl.value}:00`;
...
function updateRadarOptions(){
  radarOptionsEl.hidden=!radarSelected();
}

function radarDateObject(){
  const date=radarDateEl.value;
  const time=radarTimeEl.value || "00:00";
...
function makePlan(c){
  ...
  if(c.source==="radar"){
    ...
    note=`レーダー: ${rt.utc} / ${rt.jst}、地域: ${radarRegionEl.value}`;
  }
